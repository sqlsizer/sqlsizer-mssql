<#
.SYNOPSIS
    Wrapper script to run Find-RemovalSubset integration tests with proper module loading.
...
.PARAMETER Filter
    ...
    - "*Circular*" - run circular reference tests
    - "*Deep*" - run deep chain tests

.EXAMPLE
    .\Run-RemovalSubsetTests.ps1 -Filter "*Deep*"
    
.EXAMPLE
    .\Run-RemovalSubsetTests.ps1 -DataSize Small -Filter "*Circular*"
#>
param(
...
Write-Host "Running Find-RemovalSubset integration tests with DataSize=$DataSize, Server=$Server$filterMsg" -ForegroundColor Cyan

# Build Pester parameters
$pesterParams = @{
    Path = "$PSScriptRoot\Find-RemovalSubset.Integration.Tests.ps1"
    Output = 'Detailed'
    PassThru = $true
}
