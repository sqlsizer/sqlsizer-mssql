# SqlSizer-MSSQL Examples (New)

This directory contains organized examples demonstrating various SqlSizer-MSSQL features and capabilities.
...
All examples in this directory use the **fully modernized API** with no legacy Color enum dependencies:

### Query Definition (Start Sets)
- **Query2** class with `State` property (TraversalState enum) - for defining initial data sets
- **Initialize-StartSet-Refactored** function - respects Query2.State property
...
|------------|------------|---------|
| `Query` with `.Color` | `Query2` with `.State` | Define start set queries |
| `Initialize-StartSet` | `Initialize-StartSet-Refactored` | Initialize start set with state |
| `ColorMap` | `TraversalConfiguration` | Configure traversal rules |
| `[Color]::Yellow` | `[TraversalState]::Pending` | Forward traversal |
| `[Color]::Blue` | `[TraversalState]::InboundOnly` | Removal traversal |

For legacy examples using `Query` and `ColorMap`, see the [Examples](../Examples/) folder and the [ColorMap Modernization Guide](../docs/ColorMap-Modernization-Guide.md).
