function Disable-ReachableIndexes
{
    [cmdletbinding()]
    param
    (
    ...
    Write-Progress -Activity "Disabling reachable indexes" -PercentComplete 0
    ...
    foreach ($tableInfo in $reachable)
    {
        ...
        foreach ($index in $table.Indexes)
        {
            ...
            if ($isPk -eq $false)
            {
                $sql  = "ALTER INDEX $($index.Name) ON $($table.SchemaName).$($table.TableName) DISABLE"
                $null = Invoke-SqlcmdEx -Sql $sql -Database $Database -ConnectionInfo $ConnectionInfo
            }
        }
    }

    Write-Progress -Activity "Disabling reachable indexes" -Completed
}
